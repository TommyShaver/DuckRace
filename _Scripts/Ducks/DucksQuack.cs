    ...
    public void DuckSFX(SampledClips clips)
    {
        switch (clips)
        {
            ...
            case SampledClips.quackHappySFX:
                PlaySFX(quackSmaple[UnityEngine.Random.Range(0, quackSmaple.Length)], quackMachine, 1.5f, .5f);
                break;
            case SampledClips.quackSadSFX:
                PlaySFX(quackSmaple[UnityEngine.Random.Range(0, quackSmaple.Length)], quackMachine, .8f, .5f);
                break;
            case SampledClips.SplashSFX:
                PlaySFX(splashSFX, splashMachine, 1f, .5f);
                break;
            case SampledClips.RocketSFX:
                PlaySFX(rocketSFX, splashMachine, 1, 1f);
                break;
            case SampledClips.IntertubeSpawnSFX:
                PlaySFX(intertubeSpawnSFX, splashMachine, 1, 1f);
                break;
            case SampledClips.IntertubeDespawnSFX:
                PlaySFX(intertubeDespawnerSFX, splashMachine, 1, 1f);
                break;
            case SampledClips.getItemSFX:
                PlaySFX(getItemSFX, splashMachine, 1, .4f);
                break;
        }
    }
