...
public class DuckManager : MonoBehaviour, DuckInterractionInterface, ICrossGoal
{
    ...
    {
        GameManager.instance.DucksCrossedFinishLine(duckText.GetName(),duckHats.CurrentHat(),duckColor.WhatColor(),duckColor.IsRainbow());
        ...
        if (ishappening)
        {
            switch (what)
            {
                ...
                case "GotItem":
                    ...
                    PlayerWarningTextScript.instance.SwitchPlayerWarningText(myName + ": Got item " + duckItems.WhatItem());
                    break;
            }
        }
        else
