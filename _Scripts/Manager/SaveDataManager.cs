...
public class SaveDataManager : MonoBehaviour
{
    ...
    public float sfxVolumeToSave = 1f;
    public float musicVolumeToSave = 1f;
    ...
    public void UnBanPlayerFromJoining(string name)
    {
        banPlayers.Remove(name);  
        SaveGameData();  
    }
    public bool CheckBannedPlayers(string name) //Check to see if player is banned?
    {
        Debug.Log(banPlayers.Contains(name) + ": name there");
    ...
    public void LoadDataOnLunach()
    {
        if (File.Exists(savePath))
        {
            ...
            firstLoad = data.firstLoaded;
            sfxVolumeToSave = data.sfxVolume;
            musicVolumeToSave = data.musicVolume;
