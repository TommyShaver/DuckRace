...
public class BoulderOpstical : MonoBehaviour
{
    ...
    {
        UIButtonScript.OnUIButtonSwitch += DestoryRock;
        GameManager.OnResetPlayers += GetRidOfRock;
        GameManager.OnClearPlayers += GetRidOfRock;
    }
    ...
    private void DestoryRock(bool isTrue, string command)
    {
        if (!isTrue && command == "rock")
            Destroy(gameObject);
    }

    private void GetRidOfRock()
    {
        Destroy(gameObject);
    }
